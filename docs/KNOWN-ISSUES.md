...
#### ✅ 論理フレームの来ない反復で生入力を進めてしまい、キー入力が落ちる

`Core/Game.cs:184`

`FixedUpdate=true` だと、更新ループの反復と論理フレームは 1:1 ではない。更新ループは論理レートを
律速にせず全力で回るので、実測で 60Hz に対しておよそ 30000 回/秒――500 反復に 1 回しか
`game.Update()` が走らない。それなのに `_inputBridge.PreUpdate()` は反復ごとに呼ばれていた。
押下エッジは生入力を 1 回進めるごとに 1 反復しか立たないため、そのエッジはほぼ確実に
`game.Update()` の走らない反復で消費され、ゲームからはキーが一度も押されなかったことになる。

TaikoFine v10（`FixedUpdate=true, FixedUpdateHz=60`、描画 1000fps 超）でタイトルの Enter が
効かなかったのはこれ。Sandbox は `FixedUpdate` が既定の false（1 反復 = 1 論理フレーム）なので
一度も踏まなかった。セルフテストも `DeterministicMode` が `LockStep` を強制するため同様に
1 反復 = 1 論理フレームで、この経路を一度も通っていなかった。

修正済み。`IsLogicStepDue()` を入力の前進より前に置き、論理フレームの来ない反復は入力を
一切進めずに捨てる。実時間の積み上げもそこへ移した（1 反復につき 1 回だけ積むため）。
可変 dt とロックステップは 1 反復 1 論理フレームなので素通しで、`--replay` はずれない。
検証用に `--no-lockstep` を足してある（`DeterministicMode` でもロックステップに倒さない）。
この経路を開けた状態で走らせると、修正前の Sandbox のセルフテストは 31 件中 16 件落ちる。
...
修正済み。押下の立ち上がりを `Core/KeyEdgeBuffer.cs` に件数として溜め、`Update()` が 1 フレームに
1 件ずつ「押下 → 解放」の順で吐き出すようにした。DxLib / RayLib どちらのバックエンドもこの共通
バッファを使う。取り込みと確定が 1:1 で交互に走る単一スレッド構成では遷移が以前と完全に同じなので、
`--replay` はずれない。Sandbox のセルフテストに取りこぼしの検証を 6 件足してある
（合成入力は `InputBridge` が押下集合からエッジを作るのでこの経路を通らない。だから見本帳シーンを
`--mt` で走らせても再現せず、バッファ本体を直接叩いて確かめている）。
