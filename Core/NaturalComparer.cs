...
public class NaturalComparer : IComparer<string>, IComparer
{
    ...
    public NaturalComparer()
        : this(NaturalSortOrder.Ascending, NaturalComparerOptions.Default, [])
    {
    }
    ...
    public NaturalComparer(NaturalSortOrder order)
        : this(order, NaturalComparerOptions.Default, [])
    {
    }
    ...
    public virtual NaturalSortOrder SortOrder
    {
        get => _order switch
        {
            1 => NaturalSortOrder.Ascending,
            -1 => NaturalSortOrder.Descending,
            _ => NaturalSortOrder.Ascending,
        }; set => _order = value switch
        {
            NaturalSortOrder.Ascending => 1,
            NaturalSortOrder.Descending => -1,
            _ => 0,
        };
    }
    ...
    /// </list>
    /// </returns>
    int IComparer.Compare(object? s1, object? s2) => LocalCompare(s1?.ToString(), s2?.ToString()) * _order;
    ...
    protected virtual int LocalCompare(string? s1, string? s2)
    {
        ...
        return p1 >= s1.Length ? p2 >= s2.Length ? 0 : -1 : 1;
    }
    ...
    private static CharTypes GetCharType(char c, char back, CharTypes state)
    {
        // ASCIIアラビア数字 (0～9)
        if (c is >= '0' and <= '9')
        {
        ...
        // ローマ数字
        else if (c is >= (char)0x2160 and <= (char)0x217F)
        {
    ...
    private static void ConvertHalf(StringBuilder source)
    {
    ...
    /// <param name="source">変換元の文字列を表す <see cref="StringBuilder"/>。</param>
    private static void ConvertUpperCase(StringBuilder source)
    {
        for (int i = 0; i < source.Length; i++)
        {
            // ASCII小文字
            if (source[i] is >= 'a' and <= 'z')
            {
                source[i] = char.ToUpperInvariant(source[i]);
            }
            // 全角小文字
            else if (source[i] is >= 'ａ' and <= 'ｚ')
            {
                source[i] = (char)(source[i] - 'ａ' + 'Ａ');
            }
        }
    }
    ...
    /// <param name="source">変換元の文字列を表す <see cref="StringBuilder"/>。</param>
    private static void ConvertKatakana(StringBuilder source)
    {
        for (int i = 0; i < source.Length; i++)
        {
            if (source[i] is >= 'ぁ' and <= 'ゞ')
            {
                source[i] = (char)(source[i] + 'ァ' - 'ぁ');
            }
            else if (source[i] is >= 'ｦ' and <= 'ﾟ')
            {
                bool replaced = false;
    ...
    private static bool GetNumber(string source, CharTypes type, ref int pos, out long value)
    {
    ...
    private static int CompareChar(char c1, char c2)
    {
        ...
        return p1 >= 0 && p2 >= 0 ? p1 - p2 : StringComparer.CurrentCulture.Compare(c1.ToString(), c2.ToString());
    }
    ...
    private class NumberConverter : INumberComverter
    {
        ...
        public bool AddChar(char number)
        {
            ...
            if (number >= _numberZero && number <= _numberNine)
            {
                ...
                Value = Value * 10 + (number - _numberZero);
            }
            // 3桁区切りのカンマかどうか
    ...
    private class RomanNumberConverter : INumberComverter
    {
        ...
        public RomanNumberConverter(char alpha)
        {
            ...
            else if (alpha is >= 'a' and <= 'z')
            {
                _alphaA = 'a';
            }
            else if (alpha is >= 'Ａ' and <= 'Ｚ')
            {
                _alphaA = 'Ａ';
            }
            else if (alpha is >= 'ａ' and <= 'ｚ')
        ...
        protected long Parse(char alpha) => (alpha - _alphaA) switch
        {
            08 => 1,// I
            21 => 5,// V
            23 => 10,// X
        ...
        protected static bool IsAlpha(char alpha) => alpha is >= 'A' and <= 'Z' or >= 'a' and <= 'z' or >= 'Ａ' and <= 'Ｚ' or >= 'ａ' and <= 'ｚ';
    ...
    private class JpRomanNumberConverter : INumberComverter
    {
        ...
        public JpRomanNumberConverter(char roman)
        {
            ...
            if (roman is >= (char)0x2160 and <= (char)0x216F)
            {
                _romanOne = (char)0x2160;
            }
            // 全角ローマ数字(ⅰ～xii,l,c,d,m)
            else if (roman is >= (char)0x2170 and <= (char)0x217F)
            {
        ...
        /// <returns>変換後の数値を表す long。</returns>
        protected long Parse(char roman) => (roman - _romanOne) switch
        {
            0x0 => 1,// I
            0x4 => 5,// V
            0x9 => 10,// X
    ...
    private class CircleNumberConverter : INumberComverter
    {
        ...
        {
            // ①～⑳
            if (number is >= (char)0x2460 and <= (char)0x2473)
            {
                Value = number - 0x2460 + 1;
            }
            // (1)～(12)
            else if (number is >= (char)0x2474 and <= (char)0x2487)
            {
                Value = number - 0x2474 + 1;
            }
            // 1.～20.
            else if (number is >= (char)0x2488 and <= (char)0x249B)
            {
                Value = number - 0x2488 + 1;
            }
            // 丸付き21～35
            else if (number is >= (char)0x3251 and <= (char)0x325F)
            {
                Value = number - 0x3251 + 21;
            }
            // {一}～{十}
            else if (number is >= (char)0x3220 and <= (char)0x3229)
            {
                Value = number - 0x3220 + 1;
            }
            // 丸付き一～十
            else if (number is >= (char)0x3280 and <= (char)0x3289)
            {
                Value = number - 0x3280 + 1;
            }
        }
    ...
    private class KanjiNumberConverter : INumberComverter
    {
        ...
        /// <returns>変換後の数値を表す long。</returns>
        protected static long Parse(char kanji) => kanji switch
        {
            '〇' => 0,
            '一' => 1,
            '二' => 2,
